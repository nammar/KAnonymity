    ...
    public class KAnonymity
    {
        ...
        public string processWSCLInteractions_PutInArray(string strWSCLContents)
        {
            ...
            catch (Exception ex)
            {
                ...
                File.AppendAllText(@dataFile, "KAnonymity Class, processWSCLInteractions_PutInArray: " + ex.Message.ToString());
        ...
        public string processWSCLTransitions_PutInArray(string strWSCLContents)
        {
            ...
            catch (Exception ex)
            {
                ...
                var dataFile = System.Web.HttpContext.Current.Server.MapPath("~/App_Data/ErrorLog.txt");
                File.AppendAllText(@dataFile, "KAnonymity Class, processWSCLTransitions_PutInArray: " + ex.Message.ToString());
        ...
        public string populate_WSTransitionsTable()
        {
            ...
            try
            {
                ...
                SqlCommand cmd = new SqlCommand("spAdd_WSCL_Transition");
                cmd.CommandType = CommandType.StoredProcedure;
                SqlDataReader reader;
                SqlParameter param1 = new SqlParameter("@source_method_name", SqlDbType.NVarChar);
                SqlParameter param2 = new SqlParameter("@destination_method_name", SqlDbType.NVarChar);
                cmd.Connection = con;

                for (int i = 0; i < arrCount; i++)
                {                   
                    cmd.Parameters.Add(param1).Value = arrOfSourceAndDest[i, 0];
                    cmd.Parameters.Add(param2).Value = arrOfSourceAndDest[i, 1];
                    con.Open();
        ...
        public string populate_WSTransitionsAllPossibleRoutes_Table()
        {
            ...
            try
            {
                ...
                for (int i = 0; i < arrCount; i++)
                {
                    WSTransitionsAllPossibleRoutes_InitialSetOfInserts(arrOfSourceAndDest[i, 0], arrOfSourceAndDest[i, 1]);
                }

                //***********************************

                // we keep iterating until we have no additional routes to add to the database
                // every time we add rows we need to iterate at least once more to see if more rows need to be added
                string strReturnMessage = "Initial Value"; // dummy value
                while (strReturnMessage != "No Rows Added")
                {
                    WSTransitionsAllPossibleRoutes_InsertsToCoverAllRoutes(ref strReturnMessage);
                }
            }
            catch (Exception ex)
            {
                ...
                var dataFile = System.Web.HttpContext.Current.Server.MapPath("~/App_Data/ErrorLog.txt");
                File.AppendAllText(@dataFile, "KAnonymity Class, populate_WSTransitionsAllPossibleRoutes_Table: " + ex.Message.ToString());
            }
        ...
        public void WSTransitionsAllPossibleRoutes_InitialSetOfInserts(string strSourceMethodName, string strDestinationMethodName)
        {
            try
            {
                ...
                SqlCommand cmd = new SqlCommand("spAdd_WSCL_Transition_AllPossibleRoutes");
                cmd.CommandType = CommandType.StoredProcedure;
                SqlDataReader reader;
                SqlParameter param1 = new SqlParameter("@ws_id", SqlDbType.Int);
                SqlParameter param2 = new SqlParameter("@source_method_name", SqlDbType.NVarChar);
                SqlParameter param3 = new SqlParameter("@destination_method_name", SqlDbType.NVarChar);
                cmd.Connection = con;

                cmd.Parameters.Add(param1).Value = intWebServiceID;
                cmd.Parameters.Add(param2).Value = strSourceMethodName;
                cmd.Parameters.Add(param3).Value = strDestinationMethodName;
                con.Open();
                reader = cmd.ExecuteReader();
                reader.Read();
            ...
            {
                var dataFile = System.Web.HttpContext.Current.Server.MapPath("~/App_Data/ErrorLog.txt");
                File.AppendAllText(@dataFile, "KAnonymity Class, WSTransitionsAllPossibleRoutes_InitialSetOfInserts: " + ex.Message.ToString());
            }
        }
