...
public class WS1_Implementation : IWebService1
{
    // declare a variable for the proxy
    private WebService3.WebService3Client proxy;
    ...
    public string[][] getPrivacyPolicyArray(int WS_ID, int Target_WS_ID, string strTargetMethodName, ref Guid gid, ref string strIncentive)
    {
        ...
        // user is submitting GUID, so give them next available incentive (following previous
        // one offered for this GUID)

        //1) insert row to GUID_IncentivesOffered
        //2) return Incentive string to user
        ...
        catch (Exception ex)
        {
            ...
            File.AppendAllText(@dataFile, "WS1, getPrivacyPolicyArray: " + ex.Message.ToString());
        }            
